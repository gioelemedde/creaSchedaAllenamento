...
const WorkoutScheduleCreator = () => {
  ...
  const generatePDF = () => {
    ...
    pdf.setFontSize(12);
    pdf.text(`Data inizio: ${new Date(schedule.startDate).toLocaleDateString()}`, 20, yOffset + 10);
    ...
    schedule.days.forEach((day) => {
      ...
      pdf.text(day.name, (pdf.internal.pageSize.width - dayNameWidth) / 2, yOffset);
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {schedule.days.map((day, dayIndex) => (
        <div key={dayIndex} className="bg-black100 shadow rounded-lg p-6 mb-6">
          <h2 className="text-xl font-bold mb-4 text-center text-white">{day.name}</h2>
          
          <table className="w-full">
            <thead>
              <tr>
                <th className="text-left p-2 border-b bg-orange-600 text-white">Esercizio</th>
                <th className="text-left p-2 border-b bg-orange-600 text-white">Serie</th>
                <th className="text-left p-2 border-b bg-orange-600 text-white">Minuti Riposo</th>
                <th className="text-left p-2 border-b bg-orange-600 text-white">Video URL</th>
